...
Work through the four steps and their human-review checkpoints:
...
3. **Wire up the sync** — copy `scaffold/` as the starting point; keys, test agenda first; follow the run ladder (dry-run → bounded run → full run → idempotency re-run).
4. **Quality control** — compare published events against the source baseline (`yarn qc` mechanizes the gathering); a human reviews `QC.md` before promotion to production.
